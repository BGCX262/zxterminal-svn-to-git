...
CMutex g_mtxErrorLock(FALSE, "ErrorLock");
...
//		HRESULT hr = jet->CompactDatabase(_bstr_t(strDatabaseSource),
//			_bstr_t(strDatabaseDestination));
//		::CoUninitialize();
		return TRUE/*hr == S_OK*/;
	}
	catch(.../*_com_error &e*/) 
...
BOOL CADODatabase::Execute(LPCTSTR lpstrExec, CADORecordset &Rst)
{
	ASSERT(m_pConnection != NULL);
	ASSERT(strcmp(lpstrExec, _T("")) != 0);
	_variant_t vRecords;
	...
	{
		m_pConnection->CursorLocation = adUseClient;
		Rst.m_pRecordset = m_pConnection->Execute(_bstr_t(lpstrExec), &vRecords, adOptionUnspecified);
		m_nRecordsAffected = vRecords.iVal;
	}
	catch(_com_error &e)
	{
...
BOOL CADORecordset::GetChunk(FieldPtr pField, CString& strValue)
{
	...
	try
	{
		...
		if( varChunk.vt!=VT_NULL && varChunk.vt!=VT_EMPTY )
...
BOOL CADORecordset::GetChunk(FieldPtr pField, LPVOID lpData)
{
	...
	try
	{
        ...
        for(long lIndex = 0; lIndex < lngSize; lIndex++)
        {
            hr= SafeArrayGetElement(varChunk.parray, &lIndex, &chData);
            if(SUCCEEDED(hr))
				((UCHAR*)lpData)[lIndex] = chData;
            else
			{
				TRACE("WARNING:Read error from the chunk data.\n");
				return FALSE;
			}
        }
	}
...
BOOL CADOCommand::AddParameter(CString strName, int nType, int nDirection, long lSize, _variant_t vtValue, int nPrecision, int nScale)
{
	try
	{
		_ParameterPtr pParam = m_pCommand->CreateParameter(strName.AllocSysString(),
			(ADODB::DataTypeEnum)nType, (ADODB::ParameterDirectionEnum)nDirection,
			lSize, vtValue);
