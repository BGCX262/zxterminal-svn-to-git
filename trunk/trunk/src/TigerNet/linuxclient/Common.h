...
{
public:
	CPackHead(unsigned char btType,unsigned char btCompress,unsigned int ulLength)
	{
		m_btPrefix = 0x55;
		m_btVersion = 1;
		m_btType = btType;
	...
	unsigned int size()
	{
		return 4 * sizeof(unsigned char) + sizeof(unsigned int);
	}
	...
	unsigned int m_ulLength;
};

class CPackBody
{
public:
	CPackBody(unsigned char btType,unsigned char* pData,unsigned int ulLen)
	{
		if (ulLen != 0)
		{
...
public:
	...
	CPacket(unsigned char btType,unsigned char btCompress,unsigned char* pData,unsigned int ulLen):m_Head(btType,btCompress,ulLen),
...
public:
	...
	CNonMDFPacket(unsigned char* pData,unsigned int unLen)
	{
		if (unLen != 0)
		{
